...
export class VoiceChannel implements Channel {
  ...
  async sendMessage(_jid: string, text: string): Promise<void> {
    if (this.pending) {
      ...
      logger.debug(
        'Voice sendMessage called with no pending request, ignoring',
      );
    }
  }
  ...
  private handleRequest(
    ...
    req.on('end', () => {
      ...
      try {
        ...
        res.end(
          JSON.stringify({
            error: 'Invalid request — expected {"text": "..."}',
          }),
        );
        return;
      }
...
registerChannel('voice', (opts: ChannelOpts) => {
  const enabled = process.env.VOICE_ENABLED || envConfig.VOICE_ENABLED;
